...
def get_train_transforms(tuner_params):
    return A.Compose(
        [
            #A.RandomSizedCrop(min_max_height=(800, 800), height=1024, width=1024, p=0.5),
            A.OneOf([
                A.HueSaturationValue(hue_shift_limit=tuner_params['hue_shift_limit'], sat_shift_limit= tuner_params['sat_shift_limit'],
                                     val_shift_limit=tuner_params['val_shift_limit'], p=0.9),
                A.RandomBrightnessContrast(brightness_limit=tuner_params['brightness_limit'],
                                           contrast_limit=tuner_params['contrast_limit'], p=0.9),
            ],p=0.9),
            A.ToGray(p=0.01),
            A.HorizontalFlip(p=0.5),
            A.VerticalFlip(p=0.5),
            A.Resize(height=1024, width=1024, p=1),
            A.Cutout(num_holes=tuner_params['num_holes'], max_h_size=64, max_w_size=64, fill_value=0, p=0.5),
            ToTensorV2(p=1.0),
        ],
        p=1.0,
...
TRAIN_ROOT_PATH = '/content/gdrive/My Drive/Colab Notebooks/globalwheat/input/global-wheat-detection/train'
...
class DatasetRetriever(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.test or random_num < 0.3:
...
class Fitter:
    ...
    def __init__(self, model, device, config,params):
        self.config = config
        self.epoch = 0
        ...
        if optimizer=="SGD":
            ...
            self.optimizer = torch.optim.ASGD(self.model.parameters(), lr=config.lr)
...
def run_training(tuner_params):
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    net.to(device)
    TrainGlobalConfig.lr = tuner_params['lr']
    ...
    fitter = Fitter(model=net, device=device, config=TrainGlobalConfig,params=tuner_params)
...
if __name__ == '__main__':
    try:
        ...
        train_dataset = DatasetRetriever(
            ...
            marking=marking,
            transforms=get_train_transforms(tuner_params),
            test=False,
        )
